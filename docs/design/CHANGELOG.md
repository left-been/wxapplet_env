...
### 修复
- 真机每次重新采集 fpId 变化的问题：battery（电量/充电）、sensors（传感器瞬时读数）、network（网络类型/信号）为动态维度，
  已从 fpId 种子中剔除（新增 `DYNAMIC_FP_KEYS`）。
- 动态维度仍保留在 categories 中：页面展示、复制导出、模拟器弱信号判断不受影响。
- `FPVersion` 提升至 `1.2.1`（种子构成变化，与 v1.2.0 不可比）。
